<template>
	<view class="common-list animated fadeIn fast">
		<view class="common-item">
			<view class="item-left"><image :src="item.avatar" mode="aspectFit" lazy-load></image></view>
			<view class="item-right">
				<view class="item-user-info u-f u-f-jsb">
					<view class="item-user-info-left u-f">
						...
						<view class="item-user-sex u-f">
							<view class="icon iconfont tag-sex" :class="[item.sex === 1 ? 'man icon-nan' : 'woman icon-nv']">{{ '  ' + item.age }}</view>
						</view>
					</view>
					<template v-if="!item.isAttention">
						<view class="item-user-info-right u-f">
							<view class="icon iconfont icon-zengjia attention" @tap="attention(item.isAttention, index)">关注</view>
							<view class="icon iconfont icon-guanbi remove"></view>
						</view>
					</template>
				</view>
				<view class="send-time">26天前</view>
				<view class="item-content">{{ item.content }}</view>
				<template v-if="item.type === 1">
					<uni-grid :showBorder="false" square>
						<block v-for="(image, index) in item.imageList" :key="index">
							<view class="image-container">
								<uni-grid-item class="image"><image class="image-grid" :src="image" @tap="previewImage(index)"></image></uni-grid-item>
							</view>
						</block>
					</uni-grid>
				</template>
				<template v-else-if="item.type === 2">
					<view class="item-media u-f-ajc">
						<image class="video-image" :src="item.contentRef" mode="widthFix" lazy-load></image>
